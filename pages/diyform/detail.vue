<template>
	<view>
		<!-- 顶部导航 -->
		<fa-navbar :title="diyform.title || '详情'"></fa-navbar>
		<view class="u-p-30">
			<block v-for="(item, index) in fieldsList" :key="index">
				...
				<view class="u-p-b-30" v-if="item.type == 'editor'">
					<view class="title u-text-weight">{{ item.title }}:</view>
					<view class="content u-tips-color"><u-parse :html="item.value" @linkpress="navigate"></u-parse></view>
				</view>
				<!-- 时间 -->
				...
				<view class="u-p-b-30" v-if="item.type == 'image'">
					<view class="title u-text-weight">{{ item.title }}:</view>
					<view class="contents u-flex u-flex-wrap"><image :src="item.value" @click="preview(item.value)" mode="aspectFill"></image></view>
				</view>
				<!-- 多图 -->
...
export default {
	...
	methods: {
		edit() {
			this.$Router.push({
				path: '/pages/diyform/diyform',
				query: { diyname: this.diyname, id: this.id }
			});
		},
		getformShow() {
			this.$api.formShow({ id: this.id, diyname: this.diyname }).then(res => {
				if (res.code) {
