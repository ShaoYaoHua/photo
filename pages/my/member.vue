<template>
	<view>
		<!-- 顶部导航 -->
		<fa-navbar title="我的VIP会员"></fa-navbar>
		<view class="u-p-30">
			<view class="center u-p-30" :style="[{ background: theme.bgColor }]">
				<view class="u-flex">
					<view class="u-m-r-15"><u-avatar size="90" :src="vuex_user.avatar"></u-avatar></view>
		...
		<view class="">
			<view class="interests u-p-30 u-flex u-flex-wrap">
				<view class="item u-text-center" v-for="(item, index) in rightdata" :key="index">
					<view class="u-flex u-row-center"><u-image width="80rpx" height="80rpx" :src="item.image" shape="circle"></u-image></view>
		...
		<view class="vip-footer u-p-30 u-border-top">
			<u-button
				...
				type="primary"
				hover-class="none"
				:custom-style="{ backgroundColor: isDisabled ? '#ccc' : theme.bgColor, color: theme.color }"
				@click="goBuy(0)"
			>
				{{ nowLevel }}
			</u-button>
...
export default {
	...
	methods: {
		...
		// #ifndef H5
		goBuy(type) {
			if (!type && this.isDisabled) {
				// this.$u.toast('')
				return;
			}
			// #ifdef MP-BAIDU
				this.$u.toast('未支持百度小程序支付!')
				return;
			// #endif
			let data = {
		...
		goBuy(type) {
			...
			console.log(this.vipList[this.current]);
			let data = {
				paytype: this.paytype,
				level: this.vipList[this.current].level,
				days: this.pricedata[this.selectIndex[this.current]].days,
