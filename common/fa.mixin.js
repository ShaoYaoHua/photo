	...
	methods: {
		...
		navigate(e) {
			if (e.href && e.href.indexOf('http') == -1) { //不完整的链接						
				...
				// #ifdef MP
				this.$util.uniCopy({
					content: this.vuex_config.config.upload.cdnurl + e.href,
					success: () => {
				...
				// #endif
				// #ifndef MP				
				window.open(this.vuex_config.config.upload.cdnurl + e.href);
				// #endif
			}
		},
		//预览图片
		...
		cdnurl(url) {
			if (!/^((?:[a-z]+:)?\/\/|data:image\/)(.*)/.test(url)) {
				return this.vuex_config.config.upload.cdnurl + url;
			};
			return url;
		}
	}
...
export const loginfunc = {
	methods: {
		...
		success(index = 1) {
			...
			// #endif
			// #ifdef H5
			const value = uni.getStorageSync('last_page');
			//有上次页面，关闭所有页面，到此页面
			if (value) {
				uni.reLaunch({
					url: value.path + this.$u.queryParams(value.query, true, 'indices')
				});
				return;
			}
			//默认到首页
