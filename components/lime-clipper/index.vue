...
export default {
	...
	data() {
		return {
			...
			ctx: null,
            rectTyle:[
                {id:1,name:'竖向',checked:'false'},
                {id:2,name:'横向',checked:'false'},
            ],
            oldck:{}
		};
	},
	...
	mounted() {
        ...
        if(this.height > this.width){
            this.rectTyle[0].checked = true
            this.rectTyle[1].checked = false
        }else{
            this.rectTyle[0].checked = false
            this.rectTyle[1].checked = true
        }
        console.log(this.rectTyle);
		this.setClipInfo();
		if(this.image) {
	...
	methods: {
        changeType(e){
            if(e.detail.value == 1){
                // 竖向
                this.height = this.oldck.height
                this.width = this.oldck.width
            }else{
                // 横向
                this.height = this.oldck.width
                this.width = this.oldck.height
            }
            this.clipWidth = this.width / 2;
            this.clipHeight = this.height / 2;
            this.clipY = (this.sysinfo.windowHeight - this.clipHeight) / 2;
            this.clipX = (this.sysinfo.windowWidth - this.clipWidth) / 2;
        },
		setDiffData(data) {
		...
		uploadImage() {
			...
			this.upsize=b[0].size/1024;
			if(parseInt(maxsize)> b[0].size/1024)
			{
				uni.showToast({
					title: "上传图片不能小于" + maxsize + "M!",
					icon: 'none'
				});
				return;
