		...
		showTabbar() {
			if (this.tabbar.list) {
				...
				this.tabbar.list.forEach(item => {
					...
					if (path == this.pageUrl || path == '/' + this.pageUrl) {
						status = true;
						this.$u.vuex('vuex_table_title', item.text);
					}
				});
				return status;
			}
	...
	methods: {
		...
		switchTab(index) {
			...
			if (this.tabbar.list[index].path.indexOf('http') != -1) {
				...
				this.$Router.push('/pages/webview/webview');
				return;
			}
			// 发出事件和修改v-model绑定的值
			this.$emit('change', index);
			if (this.tabbar.list[index].path) {
				this.$u.vuex('vuex_table_title', this.tabbar.list[index].text);
				// #ifndef APP-PLUS
				this.$Router.replaceAll(this.tabbar.list[index].path);
				// #endif
